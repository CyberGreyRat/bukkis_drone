...
esp_err_t mpu6050_init(void) {
    ...
    if (err != ESP_OK) { ESP_LOGE(TAG, "Fehler beim Aufwecken!"); return err; }
    ...
    if (err != ESP_OK) { ESP_LOGE(TAG, "Fehler beim DLPF setzen!"); return err; }
...
esp_err_t mpu6050_read_angles(mpu6050_angles_t *angles) {
    ...
    if (err == ESP_OK) {
        ...
        angles->pitch = current_angle_pitch;
        angles->roll = current_angle_roll;
    }
